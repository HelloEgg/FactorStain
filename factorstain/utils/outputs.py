...
def prepare_output(config: dict) -> Path:
    ...
    for child in ("logs", "figures", "checkpoints", "splits", "metadata", "tables"):
...
def write_metrics(
    out: Path, rows: list[dict[str, Any]], summary: dict[str, Any]
) -> None:
...
def write_decision(
    ...
    if status not in {"STRONG_GO", "GO", "NO_GO", "GO_WITH_SCOPE_REDUCTION"}:
        raise ValueError(f"Invalid decision status: {status}")
    if not decision_valid:
        reasons = [
            "FAST_DEV_RUN is a pipeline check, not a scientific experiment.",
            *reasons,
        ]
    payload = {
        "status": status,
...
def write_report(
    out: Path, title: str, summary: dict[str, Any], decision: dict[str, Any]
) -> None:
...
def update_master(outputs_root: str | Path) -> None:
        ...
        if decision_file.exists():
            ...
            records.append(
                (
                    ...
                    decision["observed"],
                    decision.get("decision_valid", True),
                )
            )
        else:
    ...
    for i, (label, status, metric, value, valid) in enumerate(records):
        ...
        axis.add_patch(
            plt.Rectangle(
                (x - 0.075, 0.38), 0.15, 0.32, color=colors[status], alpha=0.92
            )
        )
        ...
        if not valid and status != "PENDING":
            axis.text(
                x, 0.13, "DEV RUN", color="#c92a2a", size=9, weight="bold", ha="center"
            )
    axis.set_title(
        "FactorStain — Master Experimental Dashboard",
        fontsize=20,
        weight="bold",
        pad=18,
    )
    ...
    lines = [
        "# FactorStain Master Report",
        "",
        "| Milestone | Status | Primary metric | Observed | Valid |",
        "|---|---|---|---:|---|",
    ]
    for label, status, metric, value, valid in records:
        lines.append(
            f"| {label} | {status} | {metric} | {value if value is not None else '—'} | {valid} |"
        )

...
def test_combination_split_retains_every_factor():
    ...
    validation = {(x["stain_id"], x["scanner_id"]) for x in split["validation_cells"]}
    ...
    assert {s for s, _ in heldout} <= {s for s, _ in train}
    assert {q for _, q in heldout} <= {q for _, q in train}
